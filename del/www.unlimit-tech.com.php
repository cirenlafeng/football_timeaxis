...
function www_unlimit_tech_com_GetUrlList($result, $args) 
{
	//验证返回状态
	...
	//选择队列区块
	$articles = pq('.layout-inner .layout-wrap');
	...
	foreach ($articles as $article)
	{
		$temp = $sqlBaseData;
		//内文链接
		$url = pq($article)->find('.post-featers .post-img a')->attr('href');
		$url = str_replace(' ','+',$url);//转义url里面的空格
		$temp['url'] = $url;

		//缩略图
		$thumbnail = pq($article)->find('.post-featers .post-img a img')->attr('src');
...
//抓取新闻内容
function www_unlimit_tech_com_GetBodyInfo($result, $args)
{
	//验证返回状态
    if ($result['info']['http_code'] != 200) {
	...
	switch ($status)
	{
		...
		case 0:
		{
			...
			// 解析html
			$args = www_unlimit_tech_com_ResolveHtml($args);
...
function www_unlimit_tech_com_ResolveHtml($data)
{
    ...
    //time
    $time = $article->find('.post-date-bd span')->text();
    $time = strReplaceNT($time);

    //content
    $content = $article->find('.post-content-bd')->html();
...
//分配的当前业务负责人ID
$operatorID['www.unlimit-tech.com'] = 0;

$urlInfo['www.unlimit-tech.com'] = [
	#0
	[
		'url' => 'http://www.unlimit-tech.com/category/reviews/page/1',
		'tag' => 6,
		'type' => 1,
		'diff' => 1,
	],
	#1
	[
		'url' => 'http://www.unlimit-tech.com/category/mobiles/page/1',
		'tag' => 6,
		'type' => 1,
		'diff' => 1,
	],
	#2
	[
		'url' => 'http://www.unlimit-tech.com/category/computer/page/1',
		'tag' => 6,
		'type' => 1,
		'diff' => 1,
	],
	...
	#4
	[
		'url' => 'http://www.unlimit-tech.com/category/%D8%A2%D8%AE%D8%B1-%D8%A7%D9%84%D8%A3%D8%AE%D8%A8%D8%A7%D8%B1/page/1',
		'tag' => 6,
		'type' => 1,
		'diff' => 1,
	],
